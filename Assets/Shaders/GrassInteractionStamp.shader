Shader "Hidden/Voyage/GrassInteractionStamp"
{
    SubShader { Pass { Blend One Zero ZTest Always ZWrite Off Cull Off HLSLPROGRAM
        ...
        half4 frag(V i):SV_Target { half4 old=SAMPLE_TEXTURE2D(_MainTex,sampler_MainTex,i.uv); float2 ab=_StampB.xy-_StampA.xy; float h=saturate(dot(i.uv-_StampA.xy,ab)/max(dot(ab,ab),1e-5)); float2 q=_StampA.xy+ab*h; float a=saturate(1-distance(i.uv,q)/max(_StampRadius,1e-5))*_StampStrength; return lerp(old,half4(_StampDirection.xy*0.5+0.5,1,1),a); }
        ENDHLSL } }
}
